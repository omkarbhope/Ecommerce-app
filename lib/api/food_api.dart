...
import 'package:farm_fresh/model/user.dart';
...
signup(User user, AuthNotifier authNotifier) async {
  ...
  if (authResult != null) {
    ...
    updateInfo.photoUrl = user.address;
...
_uploadFood(Food food, bool isUpdating, Function foodUploaded, {String imageUrl}) async {
  CollectionReference foodRef = Firestore.instance.collection(foodName);
...
deleteFood(Food food, Function foodDeleted) async {
  ...
  await Firestore.instance.collection(foodName).document(food.id).delete();
  foodDeleted(food);
}
